...
# then just `${DOCKER_COMMAND} pull web`
# note if you do a `${DOCKER_COMMAND} build` it will try to build everything
# but if you do a `${DOCKER_COMMAND} up` it will just use the image you pulled down
...
#${DOCKER_COMMAND} run --rm web bash -c "bundle; bundle exec rake db:create db:initial_setup"

# If you are updating an existing installation, run this:
${DOCKER_COMMAND} run --rm web bash -c "bundle; bundle exec rake db:migrate"
