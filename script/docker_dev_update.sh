...
if [[ -n "$UPDATE_CODE" ]] || [[ -n "$REBUILD_DOCKER" ]] && [[ "$(${DOCKER_COMMAND} top | wc -l)" -gt 0 ]]; then
  echo "You should probably stop docker containers before rebasing code"
  prompt "Would you like to attempt to stop containers with ${DOCKER_COMMAND} stop? [y/n]" stop
  if [[ ${stop:-n} == 'y' ]]; then
    ${DOCKER_COMMAND} stop
  else
