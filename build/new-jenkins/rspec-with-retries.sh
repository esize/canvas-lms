...
while true; do
  ...
  if [[ $reruns_started ]]; then
    if [ $1 ] && [ $1 = 'performance' ]; then
      commands+=("${DOCKER_COMMAND} exec -T canvas bundle exec rspec --options spec/spec.opts spec/selenium/performance/ --only-failures --failure-exit-code 99")
    fi
  else
    if [ $1 ] && [ $1 = 'performance' ]; then
      commands+=("${DOCKER_COMMAND} exec -T canvas bundle exec rspec --options spec/spec.opts spec/selenium/performance/ --failure-exit-code 99")
    fi
  fi
